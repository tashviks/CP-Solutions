...
//     cout<<"YES"<<endl;
    ...
    // int cst = abs(str.front() - str.back());
    // cout<<cst<<" ";
 
    // std::set<int> v;
 
    // std::map<char, set<int>> mp; 
    ...
    // {
    //     for(char ch = str.front();ch>=str.back();ch--)
 ...
 //    cout<<rem<<" "<<ans<<endl;
 // }
    int ans = r / a + r % a; // calculating the value for maximum r
    int m = abs(1-(r / a * a));  // calculating r/
    if(m >= l)
        ans = max(ans, m / a + m % a);
    cout<<ans<<endl;
}
