...
*/
        ...
        bool result = true;
        int l = s.size();
        for (int i = l - 1; i >= 0 and result; --i) // DETERMINED TERM
            if (t.back() == s[i])  // traverse determined from the back and if you find the occurence of last letter of t , pop it from t
            {
                t.pop_back();
            }
            else if (t.rfind(s[i]) != -1) // if any letter of t not found in n , result is false 
            {
                result = false;
                break;
            }
        ...
        if(result and t.empty() ) // if the string t is empty i.e it can be formed else it cant 
            cout<<"YES"<<endl;
        else
            cout<<"NO"<<endl;
