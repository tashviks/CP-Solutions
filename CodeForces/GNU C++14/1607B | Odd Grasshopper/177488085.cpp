...
void solve()
{
        ...
        // cout<<xx<<"\n";
        ll left=0,right=0;
        ll ok=ceil((double)(n-1)/2);
        // ok++;
        // cout<<ok<<"\n";
        ll okk=n-ok-1;
        if(ok%2!=0 && okk%2!=0) ok++,okk--;
        if(x%2==0)
        {
            x--;
            if(ok%2==0) right= (2*ok +1)*ok/2;
            else right= (2*(ok+1)+1)*(ok+1)/2 - (2*ok+1);
            if(okk%2==0) left= (2*okk + 5)*okk/2;
            else left= (2*(okk+1)+5)*(okk+1)/2 - (2*okk+3);
        }
        else
        {
            x++;
            if(ok%2==0) left= (2*ok +1)*ok/2;
            else left= (2*(ok+1)+1)*(ok+1)/2 - (2*ok+1);
            if(okk%2==0) right= (2*okk + 5)*okk/2;
            else right= (2*(okk+1)+5)*(okk+1)/2 - (2*okk+3);
        }
        // cout<<x<<" "<<left<<" "<<right<<"\n";
        cout<<(x-left+right)<<"\n";
