...
void solve()
{
 int n ;
    ...
    std::vector<ll> v(n) , a(n);
 
    for(auto &it : a)
        cin>>it;
 
    ll ans = 1e18;
        // Do not initialise ans to INT_MAX incase of long long 
 
        forn( i , 0 , n)
        {
            //We will make all indexes zero one by one
            //Then we make left hand indexes negative in decreasing order
            //and right hand indexes positive in increasing order greadily 
           ll prev = 0 , res = 0;
