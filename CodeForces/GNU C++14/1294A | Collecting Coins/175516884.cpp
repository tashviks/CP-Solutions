...
int cnt = 0; 
   ...
   /* int n , s = 0 , d = 0;
    ...
    forn(i , 0 , n) 
    {
        if(mp[v[i]]==1 && st.find(v[i])==st.end())
        {
            ++s;
            st.insert(v[i]);
        }
        else if(mp[v[i]]>1 && st.find(v[i])==st.end())
        {
            ++d;
            st.insert(v[i]);
        }
...
/*
    ...
    if(c.size()>d.size()){
        ...
        ans += max(2*c[c.size()-1], 2*d[d.size()-1]);
        forn(i , 0 , c.size()-1){
            ans+=2*c[i] + 2*d[i];
        }
        cout<<ans<<endl;
    ...
    sort(v.begin() , v.end()-1);
       ...
       if(ans < 0)
       {
        ...
        ans%3 == 0 ? cout<<"YES"<<endl : cout<<"NO"<<endl;
