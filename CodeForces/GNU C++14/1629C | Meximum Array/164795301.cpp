...
void solve()
{
  ll n;
        ...
        {
            curren.clear(); val = 0;
            curren.insert(arr[value]);
            while(curren.find(val) != curren.end()){
                val++;
            }
            frequency[arr[value]]--;
            if(frequency[arr[value]] == 0){
                frequency.erase(frequency.find(arr[value]));
            }
            value++;
            while(value < n and frequency.find(val) != frequency.end()){
                curren.insert(arr[value]);
                while(curren.find(val) != curren.end()){
                    val++;
                }
